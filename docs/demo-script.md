...
2. **AgentBus observability:** show Cora's live intake, grounded research,
...
3. **Control:** point out that human approval and the write gateway remain
   pending and the ticket has no AI solution note or approval tags.
4. **Native approval:** in Freshservice, let the human add the exact private
...
5. **Native proof:** refresh Freshservice and show the gateway-applied solution
   note, citations, category recommendation, and the `ai-assisted` and
   `human-approved` tags.
6. **Technical proof:** switch to the cockpit and show Cora, the native approval
   watcher, and the write gateway in one durable AgentBus reply chain.
7. **Architecture card:** explain that production would replace the Markdown
